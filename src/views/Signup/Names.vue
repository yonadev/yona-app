<template>
  <div id="names" class="header-template">
    ...
    <div class="wrapper">
      <input-floating-label :validate="{required: true}" id="firstname" :disabled="false" class="with-border-input" :label="$t('firstname')" :value.sync="firstname" type="text" icon="icn_name.svg"></input-floating-label>
      <input-floating-label :validate="{required: true}" id="lastname" class="with-border-input" :label="$t('lastname')" :value.sync="lastname" type="text" icon="icn_name.svg"></input-floating-label>
    ...
    <div class="is-centered bottom-aligned">
      <router-link class="button" :to="{name: 'Choose'}">{{$t('previous')}}</router-link>
      <div @click="validateFields()" class="button">{{$t('next')}}</div>
    </div>
  </div>
</template>
